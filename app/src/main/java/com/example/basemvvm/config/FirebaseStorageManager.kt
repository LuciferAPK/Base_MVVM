...
enum class FirebaseStorageRegion(val value: String) {
    //Châu Mỹ
...
object FirebaseStorageManager {
    ...
    private var mStorageUrl = ""

    fun getStorageUrl() = mStorageUrl

    fun setStorageUrl(context: Context, country: String) {
        mStorageUrl = getBestStorage(context, country)
        Logger.logAction("FirebaseStorageUrl for $country: $mStorageUrl")
    }

    private fun getBestStorage(context: Context, country: String): String {
        return if (FirebaseStorageRegion.AS.value.contains(country, ignoreCase = true))
            "context.getString(R.string.firebase_storage_bucket_as)"
        else if (FirebaseStorageRegion.EU.value.contains(country, ignoreCase = true))
            "context.getString(R.string.firebase_storage_bucket_eu)"
        else
            "context.getString(R.string.firebase_storage_bucket)"
    }
}