...
enum class ServerRegion(val value: String) {
    //Châu Mỹ
...
object ServerUrlManager {
    ...
    private var mServerBaseUrl = ""
    private var mServerFailedBaseUrl = ""

    fun getServerBaseUrl(): String = mServerBaseUrl

    fun getServerFailedUrl() = if (mServerFailedBaseUrl.isEmpty()) "" else "$mServerFailedBaseUrl/"

    fun setServerBaseUrl(configResult: RemoteConfigResponse, country: String) {
        Logger.logAction("ServerBaseUrl: $mServerBaseUrl")
        Logger.logAction("mServerFailedBaseUrl: $mServerFailedBaseUrl")
    }

    private fun getBestServer(configResult: RemoteConfigResponse, country: String): String {
        return ""
    }
}